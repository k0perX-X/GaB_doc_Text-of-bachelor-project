...
Так как в мои задачи не входит реализация технического сервиса, то рассмотрение компонентов данной системы рассмотрено не будет, но в мои задачи входит разработка концепции дизайна интерфейса технического сервиса и его прототипирование. 

Для проектирования и прототипирования было использовано программное обеспечение Figma. Были спроецированы следующие экраны: 
...
Прототип представлен в приложении [-@sec:figma-tech].
...
Итоговый интерфейс имеет адаптивную вёрстку и приспособлен для использования устройствами размером от 4,7 до 7 дюймов. Изображение интерфейса представлено ниже на рисунке [-@fig:map-machine].
...
Удобство использования ASP.NET для реализации API заключается в том, что фактически разработка RESTful и gRPC API ничем не отличается от разработки классической библиотеки. Требуется лишь добавить атрибуты типа запроса к функциям. К классам следует добавить атрибуты, указывающие на адрес данного класса.
...
Обновление на контейнеров на тестовом и продакшн серверах выполняются автоматически в соответствии с правилами обновления: тест раз в день, продакшн раз в неделю с опозданием в неделю, на версию, которая была помечена тегом _stable_.
...
| Критерий оценивания | Яндекс Облако | РЕГ.РУ | Cloud.ru | VK Cloud |
|:---|:---:|:---:|:---:|:---:|
...
Для создания тестового полнофункционального пространства был выбран хостинг-провайдер Яндекс Облако, так как данный провайдер предоставил кампании грант на свои услуги в размере 400 тыс. рублей на пол года, что полностью покрывает затраты на тестирование продукта.

Для развертывания пользовательского полнофункционального пространства предполагается использование хостинг-провайдера Cloud.ru, так как на данный момент еще действует грант Яндекс Облака и закупка дополнительных мощностей не требуется.
